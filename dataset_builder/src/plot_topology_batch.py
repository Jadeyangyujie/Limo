"""Generate large, five-class Teacher-A reachability figures.
...
STATE_DESCRIPTIONS = (
    "center cell has no reliable elevation/risk",
...
def plot_one(
    ...
    *,
    ...
    ego_rectangles = OmegaConf.to_container(mppi_cfg.footprint, resolve=True)
    ...
    result = build_teacher_a(
        geometry=geometry,
        known_trav=trav.known_reachability,
        risk=trav.reachability_risk,
    ...
    state_ax.set_title(
        f"Five-class reachability (obstacle-only inflation)  |  requested r={radius_m:.2f} m  "
    ...
    if result.ego_mask.any():
        state_ax.contour(
            result.ego_mask.astype(np.uint8),
            levels=[0.5],
            origin="lower",
            extent=geometry.imshow_extent_yx,
            colors=["#fff200"],
    ...
    state_handles.append(
        plt.Line2D([], [], color="#fff200", linestyle="--", linewidth=1.8, label="fixed ego mask")
    )
    ...
    return {
        ...
        "ignore_count": int(result.ignore_mask.sum()),
        "ego_mask_count": int(result.ego_mask.sum()),
        "trusted_ego_unknown_count": int(result.trusted_ego_unknown.sum()),
        "known_reachability_count": int(result.effective_known.sum()),
    }
